...
class Proxy {
  ...
  val servicesRef: AtomicReference[Map[String, Seq[Target]]] = new AtomicReference[Map[String, Seq[Target]]](
    Map(
      "test.foo.bar" -> Seq(
      ...
      case Some(rawSeq) => {
        ...
        Retry
          .retry(3) {
            ...
            val circuitBreaker = circuitBreakers.computeIfAbsent(target.url,
                                                                 _ =>
          ...
          json.as(Command.decoder) match {
            ...
            case Right(Command("ADD", domain, target)) =>
              ...
              HttpResponse(
                200,
                entity =
                  HttpEntity(ContentTypes.`application/json`, Json.obj("done" -> Json.fromBoolean(true)).noSpaces)
              )
