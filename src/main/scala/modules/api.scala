...
object PreconditionModule {
  def validatePreconditions(modules: Seq[PreconditionModule], reqId: String, service: Service, request: HttpRequest): Either[HttpResponse, Unit] = {
    var index = 0
...
object ServiceAccessModule {
  def access(modules: Seq[ServiceAccessModule], reqId: String, service: Service, request: HttpRequest): WithApiKeyOrNot = {
    var index = 0
...
trait HeadersTransformationModule extends Module {
  def transform(reqId: String, host: String, service: Service, target: Target, request: HttpRequest, waon: WithApiKeyOrNot, headers: Seq[HttpHeader]): Seq[HttpHeader]
}
object HeadersTransformationModule {
  def transform(modules: Seq[HeadersTransformationModule], reqId: String, host: String, service: Service, target: Target, request: HttpRequest, waon: WithApiKeyOrNot, headers: Seq[HttpHeader]): Seq[HttpHeader] = {
    modules.foldLeft(Seq.empty[HttpHeader])((seq, module) => seq ++ module.transform(reqId, host, service, target, request, waon, headers))
  }
}

// can handle custom template errors
trait ErrorRendererModule extends Module {
  def render(reqId: String, status: Int, message: String, service: Service, request: HttpRequest): HttpResponse
}
object ErrorRendererModule {
  def render(modules: Seq[ErrorRendererModule], reqId: String, status: Int, message: String, service: Service, request: HttpRequest): HttpResponse = {
...
object TargetSetChooserModule {
  def choose(modules: Seq[TargetSetChooserModule], reqId: String, service: Service, request: HttpRequest): Seq[Target] = {
